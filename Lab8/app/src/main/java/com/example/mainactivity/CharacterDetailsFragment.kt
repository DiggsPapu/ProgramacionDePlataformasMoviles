...
class CharacterDetailsFragment: Fragment(R.layout.characterdetails_fragment) {
    ...
    private fun setAll(){
        imageView.load(args.characterToPass.image){
        ...
        name.text = args.characterToPass.name
        species.text = args.characterToPass.species
        status.text = args.characterToPass.status
        gender.text = args.characterToPass.gender
    }
}